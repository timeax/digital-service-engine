...
function utilityErrors(all: Err[]): Err[] {
    return all.filter((e: Err) => typeof e.code === 'string' && e.code.startsWith('utility_'));
}

function opt(id: string, patch?: Partial<FieldOption>): FieldOption {
    const base: FieldOption = {id, label: id, pricing_role: 'utility' as const, meta: {}};
    return {...base, ...(patch ?? {})};
}
...
describe('utility validation', () => {
    it('flags an option marked as utility that also has a service_id (utility_with_service_id)', () => {
        ...
        expect(util.some((e) => e.code === 'utility_with_service_id' && e.nodeId === 'O1')).toBe(true);
    });
    ...
    it('flags an option marked as utility with missing/invalid rate (utility_missing_rate)', () => {
            ...
            field('F2', {
                options: [
                    opt('O2', {
                        pricing_role: 'utility',
                        // @ts-ignore
                        meta: {utility: { /* rate missing */ mode: 'per_quantity'}},
                    }),
                    opt('O3', {
                        pricing_role: 'utility',
        ...
        expect(util.some((e) => e.code === 'utility_missing_rate' && e.nodeId === 'O2')).toBe(true);
        expect(util.some((e) => e.code === 'utility_missing_rate' && e.nodeId === 'O3')).toBe(true);
    });
    ...
    it('flags an option marked as utility with invalid mode (utility_invalid_mode)', () => {
        ...
        props.fields.push(
            field('F3', {
                options: [
                    opt('O4', {
                        pricing_role: 'utility',
                        meta: {utility: {rate: 2.5, mode: 'banana' as unknown as 'flat'}}, // invalid
                    }),
                ],
            }),
        );
        ...
        expect(util.some((e) => e.code === 'utility_invalid_mode' && e.nodeId === 'O4')).toBe(true);
    });
    ...
    it('accepts per_value utilities with or without valueBy', () => {
        ...
        props.fields.push(
            field('F5', {
                options: [
                    opt('O6', {
                        meta: {utility: {rate: 0.25, mode: 'per_value', valueBy: 'value'}},
                    }),
                    opt('O7', {
