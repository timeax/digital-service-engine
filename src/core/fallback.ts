...
import type { ServiceProps, ServiceFallback, ServiceIdRef, NodeIdRef, Tag } from '../schema';
    ...
    for (const [nodeId, list] of Object.entries(fb.nodes ?? {})) {
        ...
        for (const cand of list) {
            ...
            for (const tagId of tagContexts) {
                const ok = s.requireConstraintFit ? satisfiesTagConstraints(tagId, { services }, cap) : true;
                if (ok) anyPass = true; else {
                    anyFail = true;
                    out.push({ scope: 'node', nodeId, primary, candidate: cand, tagContext: tagId, reason: 'constraint_mismatch' });
                }
            }
            // If none passed, we already added per-context mismatches above
            void anyPass; void anyFail;
        }
    }
...
function passesRate(
    ...
    switch (policy.kind) {
        case 'lte_primary': return candRate <= primaryRate;
        case 'within_pct': return candRate <= primaryRate * (1 + policy.pct / 100);
        case 'at_least_pct_lower': return candRate <= primaryRate * (1 - policy.pct / 100);
    }
}

function satisfiesTagConstraints(
    tagId: string,
    ctx: { services: DgpServiceMap },
    cap: { dripfeed?: boolean; refill?: boolean; cancel?: boolean }
): boolean {
    const tag = (ctx as any).props?.filters?.find?.((t: Tag) => t.id === tagId) as Tag | undefined;
    // Prefer constraints already propagated into props (normalise() does this)
    const eff = tag?.constraints;
    if (!eff) return true;
    // Only enforce flags explicitly set true at the tag; false means "no requirement"
...
function primaryForNode(props: ServiceProps, nodeId: string): { primary?: ServiceIdRef; tagContexts: string[]; reasonNoPrimary?: string } {
    // Tag node?
