...
function ownerOfOption(props: ServiceProps, optionId: string): { fieldId: string; index: number } | null {
...
function ensureServiceExists(opts: EditorOptions, id: any) {
    if (typeof opts.serviceExists === 'function') {
        if (!opts.serviceExists(id)) throw new Error(`service_not_found:${String(id)}`);
        return;
    }
    if (opts.serviceMap) {
...
export class Editor {
    ...
    constructor(builder: Builder, api: CanvasAPI, opts: EditorOptions = {}) {
        ...
        // @ts-ignore
        this.opts = {
            historyLimit: Math.max(1, Math.min(opts.historyLimit ?? MAX_LIMIT, 1000)),
    ...
    exec(cmd: Command): void {
        try {
            ...
            this.emit('editor:error', {message: (err as Error)?.message ?? String(err), code: 'command'});
            throw err;
        }
    }
    ...
    undo(): boolean {
        ...
        this.emit('editor:undo', {stackSize: this.history.length, index: this.index});
        return true;
    }
    ...
    redo(): boolean {
        ...
        this.emit('editor:redo', {stackSize: this.history.length, index: this.index});
        return true;
    }
    ...
    private duplicateTag(tagId: string, opts: DuplicateOptions): string {
        ...
        const collect = (t: typeof src, acc: typeof src[]) => {
            acc.push(t);
            for (const child of tags.filter(x => x.bind_id === t.id)) collect(child as any, acc);
        };
        const subtree: typeof src[] = [];
        collect(src, subtree);

        // allocate ids
        for (const n of subtree) idMap.set(n.id, n.id === src.id ? id : this.uniqueId(n.id));

        // build clones
        const clones = subtree.map(n => {
            const cloned = {...n};
            cloned.id = idMap.get(n.id)!;
            cloned.label = (n.id === src.id)
                ? label
    ...
    private duplicateField(fieldId: string, opts: DuplicateOptions): string {
        ...
        const optId = (old: string) => this.uniqueOptionId(id, (opts.optionIdStrategy ?? defaultOptionIdStrategy)(old));
        ...
        this.patchProps(p => {
            ...
            if (opts.copyOptionMaps) {
                const maps: Array<'includes_for_options' | 'excludes_for_options'> = ['includes_for_options', 'excludes_for_options'];
                for (const mapKey of maps) {
                    ...
                    for (const [key, arrIds] of Object.entries(m)) {
                        ...
                        if (kField === fieldId && kOpt && optionIdMap.has(kOpt)) {
    ...
    private duplicateOption(fieldId: string, optionId: string, opts: DuplicateOptions): string {
        const props = this.builder.getProps();
        const fields = props.fields ?? [];
        const f = fields.find(x => x.id === fieldId);
        if (!f) throw new Error(`Field not found: ${fieldId}`);
        const optIdx = (f.options ?? []).findIndex(o => o.id === optionId);
        if (optIdx < 0) throw new Error(`Option not found: ${fieldId}::${optionId}`);
        const src = (f.options ?? [])[optIdx];

        const newId = this.uniqueOptionId(fieldId, (opts.optionIdStrategy ?? defaultOptionIdStrategy)(src.id));
        ...
        this.patchProps(p => {
            ...
            if (opts.copyOptionMaps) {
                ...
                for (const mapKey of ['includes_for_options', 'excludes_for_options'] as const) {
                    const m = p[mapKey] ?? {};
                    if (m[oldKey]) {
     ...
     */
    placeNode(id: string, opts: { scopeTagId?: string; beforeId?: string; afterId?: string; index?: number }) {
        if (isTagId(id)) {
            ...
            this.exec({
                ...
                do: () => this.patchProps(p => {
                    ...
                    const siblings = all.filter(t => (t.bind_id ?? '__root__') === groupKey);
                    ...
                    if (opts.beforeId) dest = Math.max(0, siblings.findIndex(t => t.id === opts.beforeId));
                    if (opts.afterId) dest = Math.min(siblings.length, siblings.findIndex(t => t.id === opts.afterId) + 1);
                    if (dest === undefined || Number.isNaN(dest)) dest = siblings.length;
            ...
            if (!opts.scopeTagId) throw new Error('placeNode(field): scopeTagId is required');
            const fieldId = id;
            ...
            this.exec({
                ...
                do: () => this.patchProps(p => {
                    ...
                    if (opts.afterId) dest = Math.min(arr.length, arr.indexOf(opts.afterId) + 1);
    ...
    placeOption(optionId: string, opts: { beforeId?: string; afterId?: string; index?: number }) {
        if (!isOptionId(optionId)) throw new Error('placeOption: optionId must start with "o:"');
        ...
        this.exec({
            ...
            do: () => this.patchProps(p => {
                ...
                let dest = typeof opts.index === 'number' ? opts.index : undefined;
                if (opts.beforeId) dest = Math.max(0, f.options.findIndex(o => o.id === opts.beforeId));
                if (opts.afterId) dest = Math.min(f.options.length, f.options.findIndex(o => o.id === opts.afterId) + 1);
                if (dest === undefined || Number.isNaN(dest)) dest = f.options.length;
    ...
    addOption(
        ...
        this.exec({
            ...
            do: () => this.patchProps(p => {
                ...
                if (list.some(o => o.id === id)) throw new Error(`Option id '${id}' already exists`);
                // @ts-ignore
                list.push({...input, id});
            }),
            undo: () => this.api.undo(),
    ...
    updateOption(optionId: string, patch: Partial<{
        ...
        if (!isOptionId(optionId)) throw new Error('updateOption: optionId must start with "o:"');
        this.exec({
            name: 'updateOption',
            do: () => this.patchProps(p => {
                const owner = ownerOfOption(p, optionId);
    ...
    removeOption(optionId: string) {
        if (!isOptionId(optionId)) throw new Error('removeOption: optionId must start with "o:"');
        this.exec({
            name: 'removeOption',
            do: () => this.patchProps(p => {
                const owner = ownerOfOption(p, optionId);
    ...
    setService(
        ...
        this.exec({
            ...
            do: () => this.patchProps(p => {
                ...
                throw new Error('setService only supports tag ("t:*") or option ("o:*") ids');
            }),
            undo: () => this.api.undo(),
        });
    }

    addTag(partial: Omit<Tag, 'id' | 'label'> & { id?: string; label: string }) {
    ...
    updateTag(id: string, patch: Partial<Tag>) {
        ...
        this.exec({
            ...
            undo: () => this.patchProps(p => {
                p.filters = (p.filters ?? []).map(t => (t.id === id && prev ? prev : t));
            }),
        });
    }
        ...
        this.exec({
            ...
            do: () => this.patchProps(p => {
                ...
                for (const f of p.fields ?? []) {
                    if (Array.isArray(f.bind_id)) f.bind_id = f.bind_id.filter(x => x !== id);
    ...
    addField(partial: Omit<Field, 'id' | 'label' | 'type'> & { id?: string; label: string; type: Field['type'] }) {
    ...
    updateField(id: string, patch: Partial<Field>) {
        ...
        this.exec({
            ...
            undo: () => this.patchProps(p => {
                p.fields = (p.fields ?? []).map(f => (f.id === id && prev ? prev : f));
            }),
        });
    }
        ...
        if (isTagId(id)) {
            this.exec({
                ...
                do: () => this.patchProps(p => {
                    ...
                    for (const f of p.fields ?? []) {
                        if (Array.isArray(f.bind_id)) f.bind_id = f.bind_id.filter(x => x !== id) as any;
                        else if (f.bind_id === id) delete f.bind_id;
                    }
                    ...
                    for (const k of Object.keys(p.order_for_tags ?? {})) {
                        p.order_for_tags![k] = (p.order_for_tags![k] ?? []).filter(fid => (p.fields ?? []).some(f => f.id === fid));
        ...
        if (isFieldId(id)) {
            this.exec({
                ...
                do: () => this.patchProps(p => {
                    ...
                    for (const k of Object.keys(p.order_for_tags ?? {})) {
                        p.order_for_tags![k] = (p.order_for_tags![k] ?? []).filter(fid => fid !== id);
                        if (!p.order_for_tags![k].length) delete p.order_for_tags![k];
                    }
    ...
    getNode(id: string):
        ...
        if (isTagId(id)) {
            ...
            return {kind: 'tag', data: t, owners: {parentTagId: t?.bind_id}};
        }
        if (isFieldId(id)) {
            const f = (props.fields ?? []).find(x => x.id === id);
            const bind = Array.isArray(f?.bind_id) ? (f!.bind_id as string[]) : (f?.bind_id ? [f.bind_id] : []);
        ...
        if (isOptionId(id)) {
            ...
            const o = f?.options?.find(x => x.id === id);
            return {kind: 'option', data: o, owners: {fieldId: own?.fieldId}};
        }
    ...
    private wouldCreateTagCycle(p: ServiceProps, parentId: string, childId: string): boolean {
            ...
            do: () => this.patchProps((p) => {
                ...
                if (kind === 'include' || kind === 'exclude') {
                    ...
                    if (isOptionId(fromId) && isFieldId(toId)) {
                        ...
                        const maps = (p as any)[mapKey] as Record<string, string[]> | undefined;
                        const next = {...(maps ?? {})};
    ...
    setConstraint(tagId: string, flag: 'refill' | 'cancel' | 'dripfeed', value: boolean | undefined) {
    ...
    private afterMutation(command: string, _before: EditorSnapshot) {
        ...
        if (this.opts.validateAfterEach) this.emit('editor:change', {
            props: this.builder.getProps(),
            reason: 'validate',
            command
        });
        else this.emit('editor:change', {props: this.builder.getProps(), reason: 'mutation', command});
    }

    private commit(label: string) {
        const snap = this.makeSnapshot(label);
        this.pushHistory(snap);
        this.emit('editor:change', {props: snap.props, reason: 'transaction', command: this.txnLabel});
    }
    ...
    private emit<K extends keyof (EditorEvents & any)>(event: K, payload: (EditorEvents & any)[K]) {
