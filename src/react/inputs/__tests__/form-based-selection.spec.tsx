...
describe("useOrderFlow + Wrapper-style setSelections (CoinMarketCap)", () => {
    it("setSelections on dripfeed reflects in flow and reveals runs/interval", async () => {
        const services: DgpServiceMap = {} as any;

        let flow: ReturnType<typeof useOrderFlow> | null = null;
        let setSelections: ((fieldId: string, ids: string[]) => void) | null =
            null;
        ...
        function Consumer() {
            ...
            // what Wrapper uses (form api)
            const ctx = useOrderFlowContext();
            setSelections = ctx.formApi.setSelections;
        ...
        expect(typeof setSelections).toBe("function");
        ...
        // ✅ Wrapper-style: directly set selections
        await act(async () => {
            // For button fields, token should be the field id (matches includes_for_buttons key)
            setSelections!("f:dripfeed", ["f:dripfeed"]);
            await flush();
        });

        // Flow should reflect it
        ...
        // runs + interval should now be visible
        expectSameMembers(flow!.visibleGroup?.fieldIds, [
            "f:link",
            "f:quantity",
            "f:dripfeed",
        ...
        // Turn it off using setSelections([])
        await act(async () => {
            setSelections!("f:dripfeed", []);
            await flush();
        });
