...
import { View } from "@/components/tree/view";
...
export function LeftPanel() {
    ...
    if (!selRef.current) {
        selRef.current = new Selection(builder, {
            env: "workspace",
            rootTagId: "root",
        });
        const rootId =
            tags.find((t) => t.id === "root" || t.id === "t:root")?.id ??
            tags[0]?.id;
        if (rootId) selRef.current.replace(rootId);
    }
    const selection = selRef.current;
    const selectedIds = React.useMemo(
        () => Array.from(selection.all()),
        [selection, props],
    ); // props to refresh tree
        ...
        <Tabs defaultValue="layers" className="flex-1 flex flex-col min-h-0">
            <div className="px-3 pt-2 border-b">
                <TabsList>
                    ...
                    <TabsTrigger value="active-services">
                        Active Services
                    </TabsTrigger>
                ...
                <ResizablePanelGroup direction="vertical" className="min-h-0">
                    ...
                    >
                        ...
                        <div className="px-3 pb-3 text-sm text-muted-foreground">
                            ...
                            Select a tag to see fields in its visible group.
                            (Rendering comes in the next step.)
                        </div>
                    </ResizablePanel>
                </ResizablePanelGroup>
            </TabsContent>
            ...
            <TabsContent value="active-services" className="flex-1 m-0 min-h-0">
                ...
                <div className="px-3 text-sm text-muted-foreground">
                    This will list services currently mapped to tags/options.
                    (We’ll wire this after Layers.)
                </div>
            </TabsContent>
            ...
            <TabsContent value="templates" className="flex-1 m-0">
                ...
                <div className="px-3 text-sm text-muted-foreground">
                    Host-provided blueprints will appear here. (We’ll wire this
                    after Active Services.)
                </div>
            </TabsContent>
        </Tabs>
    );
