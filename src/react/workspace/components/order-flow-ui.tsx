...
export type OrderFieldsProps = {
    ...
    BeforeInputs?: React.ComponentType<{
        group: VisibleGroup;
        fields: Field[];
    }>;
...
export function OrderFlowUi({
    ...
    const { builder, selection, activeTagId, setActiveTag } =
        useOrderFlowContext();
    const props = builder.getProps() as ServiceProps;

    const vg = selection.visibleGroup();
    const fieldIds: string[] =
        vg.kind === "single" ? (vg.group.fieldIds ?? []) : [];
    ...
    const group: VisibleGroup | undefined =
        vg.kind === "single" ? vg.group : undefined;
    ...
    return (
        <div style={{ display: "grid", gap }}>
            ...
            {group && BeforeInputs ? (
                <BeforeInputs group={group} fields={fields} />
            ) : null}
            ...
            {group && AfterInputs ? (
                <AfterInputs group={group} fields={fields} />
            ) : null}
        </div>
    );
}
...
function TagChip(
    props: TagItemProps & { asChild?: React.ComponentType<TagItemProps> },
) {
